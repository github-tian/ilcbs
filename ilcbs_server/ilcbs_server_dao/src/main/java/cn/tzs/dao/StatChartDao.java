...
public interface StatChartDao extends JpaRepository<ContractProduct, String>, JpaSpecificationExecutor<ContractProduct> {
    ...
    @Query(value = "select * from (select product_no , sum(cnumber) from CONTRACT_PRODUCT_C group by product_no order by sum(cnumber) desc) where rownum < 16", nativeQuery = true)
